...
public class AdminManager {
    ...
    public synchronized boolean peekSetupToken(String token) {
        return token != null && this.setupToken != null && this.setupToken.equalsIgnoreCase(token.trim());
    }
    ...
    public synchronized String getPreparedOwnerTotp(String setupToken) {
        if (this.setupToken == null) return null;
        return pendingOwnerTotp.get(this.setupToken);
    }

    public synchronized void clearPreparedOwnerTotp(String setupToken) {
        if (this.setupToken != null) pendingOwnerTotp.remove(this.setupToken);
    ...
    public synchronized OnboardingResult completeMasterOnboarding(
        ...
        String pwdErr = PasswordUtils.validateStrength(newPassword);
        if (pwdErr != null) {
    ...
    public synchronized RegisterResult registerCandidate(String username, String password, String totpSecret, String totpCode) {
        ...
        String pwdErrReg = PasswordUtils.validateStrength(password);
        if (pwdErrReg != null) {
            return new RegisterResult(RegisterStatus.INVALID_INPUT, pwdErrReg, List.of());
        }
